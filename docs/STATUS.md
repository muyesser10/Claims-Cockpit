...
- **Sıradaki iş (BE):** CLAUDE.md §2/§4 güncellemesi (borç), soyisim sözlüğü resmi kaynağı (Müyesser'den bekleniyor), **`claims_flat` view migration'ı** (tanım hazır: `migrations/sql/claims_flat.sql`), **ADR-003 onayı** (compose'a `rag` servisi eklendi, aynı gün merge gerekiyor — CLAUDE.md §5), `monitoring/prometheus.yml`'ye `rag` scrape hedefi.
...
- [ ] **ADR-003 ekip onayı** — durum hâlâ `Proposed`. `docker-compose.yml`'ye `rag` servisi eklendi (ortak dosya, CLAUDE.md §5) ve `.env.example`'a `RAG_SERVICE_URL` girdi; onay + aynı gün merge gerekiyor
